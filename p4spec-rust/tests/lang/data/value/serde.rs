...
fn test_independent_native_payload_survives_source_arena_drop() {
    ...
    let value = make::external_payload(
        &mut arena,
        typ::TypKind::Text.into(),
        native.clone(),
        span_at(52),
...
fn test_independent_export_preserves_stored_relative_extern_json() {
    ...
    let value_external =
        make::external_payload(&mut arena, typ::TypKind::Text.into(), native, span_at(55)).unwrap();
    ...
    assert_eq!(
        get::external(&arena_decoded, &value_decoded).unwrap(),
        &json_native
    );
    assert_eq!(
        payload::encode(&arena_decoded, &value_decoded).unwrap(),
...
fn test_native_json_differs_by_mode_and_relative_snapshots_preserve_handles() {
    ...
    let value_relative = make::external_payload(
        &mut arena,
        typ::TypKind::Text.into(),
        native_relative,
        Span::default(),
    ...
    .unwrap();
    let value_independent = make::external_payload(
        &mut arena,
        typ::TypKind::Text.into(),
        native_independent,
        Span::default(),
    ...
    let value_changed = make::external_payload(
        &mut arena,
        typ::TypKind::Text.into(),
        native_changed,
        Span::default(),
    ...
    let value_restored = make::external_payload(
        &mut arena,
        typ::TypKind::Text.into(),
        native_restored,
        Span::default(),
...
fn test_relative_native_canonical_equality_retains_nested_value_handles() {
    ...
    for value in [value, value_retyped, value_relocated] {
        ...
        let native = Rc::new(payload::encode_with(&arena, ArenaRelative, &register).unwrap());
        let value_external = make::external_payload(
            &mut arena,
            typ::TypKind::Text.into(),
            native,
            Span::default(),
...
fn test_independent_native_equality_retains_value_annotations() {
    ...
    for value_other in [value_retyped, value_relocated] {
        ...
        let value_external_other = make::external_payload(
            &mut arena,
            typ::TypKind::Text.into(),
            native_other,
            Span::default(),
...
fn test_independent_extern_equality_and_hash_follow_original_json() {
    ...
    for json_a in &jsons {
        for json_b in &jsons {
            ...
            let value_a = make::external_payload(
                &mut arena,
                typ::TypKind::Text.into(),
                native_a,
                Span::default(),
            ...
            .unwrap();
            let value_b = make::external_payload(
                &mut arena,
                typ::TypKind::Text.into(),
                native_b,
                Span::default(),
...
fn test_direct_external_fields_preserve_opaque_json_in_both_modes() {
    ...
    for encoding in [ArenaIndependent, ArenaRelative] {
        ...
        .unwrap();
        assert_eq!(get::external(&arena, &value).unwrap(), json.as_ref());
        assert!(
    ...
    std::thread::Builder::new()
        ...
        .spawn(|| {
            for opaque in [true, false] {
                ...
                if opaque {
                    let mut json_inner = get::external(&arena_decoded, &value_decoded).unwrap();
                    for _ in 0..512 {
