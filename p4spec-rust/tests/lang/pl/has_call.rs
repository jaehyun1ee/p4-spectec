use p4spec_rust::lang::{
    ...
    pl::{annot::Annotated, ast},
    traits::has_call::HasCall,
};

fn exp(kind: ast::ExpKind) -> ast::Exp {
    Annotated::new(p4spec_rust::note_phrase! {
        node: kind,
...
fn id(name: &str) -> ast::Id {
    p4spec_rust::phrase!(node: name.into(), span: Span::default())
}
...
fn test_update_path_call_is_detected() {
    ...
    let ast::ExpKind::Upd(_, path, _) = &exp_update.node.node else { unreachable!() };
...
fn call(name: &str, line: usize, args: Vec<ast::Arg>) -> ast::Exp {
    ...
    exp_call.node.span = Span::new(pos.clone(), pos);
    exp_call
}
...
fn test_nested_calls_retain_original_expressions_in_preorder() {
    ...
    assert_eq!(
        exps_call
            .iter()
            .map(|exp| exp.node.span.left.line)
            .collect::<Vec<_>>(),
        [10, 20, 30]
    );
    let ast::ExpKind::Tuple(exps) = &exp_tuple.node.node else { unreachable!() };
    let ast::ExpKind::Call(_, _, args) = &exps[0].node.node else { unreachable!() };
