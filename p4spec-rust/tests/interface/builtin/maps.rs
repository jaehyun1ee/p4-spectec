...
fn test_map_update_retains_notation() {
    ...
    let pairs =
        make::list(&mut arena, typ_pairs.node.clone().into(), Vec::new(), span.clone()).unwrap();
    ...
    let result =
        maps::add_map(&mut arena, &[typ_key.clone(), typ_value.clone()], &[map, key, value])
            .unwrap();
    let mut span_key = Span::default();
    span_key.left.line = 17;
    let key_updated = Value { span: make::bool(&mut arena, false, span_key).unwrap().span, ..key };
    let value_updated = make::bool(&mut arena, false, Span::default()).unwrap();
    let result =
        maps::add_map(&mut arena, &[typ_key, typ_value], &[result, key_updated, value_updated])
            .unwrap();
    ...
    let Mixfix::Arg(pairs) = inner.as_ref() else { panic!("expected map entries") };
    let pairs = get::list(&arena, pairs).unwrap();
    assert_eq!(pairs.len(), 1);
    assert_eq!(*get::case(&arena, &pairs[0]).unwrap().args()[1], value_updated);
    let Mixfix::Seq(items) = get::case(&arena, &pairs[0]).unwrap() else {
        panic!("expected pair notation")
    };
    let Mixfix::Atom(colon) = &items[1] else { panic!("expected colon") };
