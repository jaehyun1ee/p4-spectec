...
fn wrapped_text(arena: &mut ValueArena) -> Value {
    ...
    make::case_(arena, &wrapper_type, value_case, Span::default()).unwrap()
}
...
fn test_unparses_scalar_and_container_values() {
    ...
    assert_eq!(unparser.render(&arena, &value).unwrap(), "true");
    let value = make::nat(&mut arena, Natural::from(42_u64), span.clone()).unwrap();
    assert_eq!(unparser.render(&arena, &value).unwrap(), "42");
    let value = make::text(&mut arena, "a\n\"b".into(), span.clone()).unwrap();
    assert_eq!(unparser.render(&arena, &value).unwrap(), "a\\n\\\"b");
    ...
    let tuple = make::tuple(&mut arena, &tuple_type, values, span).unwrap();
...
fn test_unsupported_values_return_typed_errors() {
    let mut arena = ValueArena::new();
    let structure =
        make::structure(&mut arena, &typ::make::bool(), Vec::new(), Span::default()).unwrap();
    assert_eq!(
