...
use super::{id, id_exp, instr, ret, span};
use crate::lang::{
...
fn test_prettification_reaches_fixed_point() {
    let (exps_match, block, block_else) =
        pretty_rel(vec![id_exp("_x'''")], vec![ret("_x'''")], Some(vec![ret("_x'''")])).unwrap();
    ...
    assert_eq!(exps_match[0], id_exp("x"));
}

#[test]
fn test_function_inputs_main_and_else_share_names_and_preserve_def_arguments() {
    let arg_exp = crate::phrase! {node: ArgKind::Exp(Box::new(id_exp("_x'''"))), span: span(4)};
    let arg_def = crate::phrase! {node: ArgKind::Def(id("_func'''")), span: span(5)};
    let instr_debug =
        instr(InstrKind::Debug(DebugInstr { exp: id_exp("_x'''"), instr: Box::new(ret("_x'''")) }));
...
fn test_prettification_moves_unchanged_expression_payloads() {
    ...
    let ptr_body = id_of_exp(return_exp(&instr_return)).node.as_ptr();
    let (_, block, _) =
        pretty_rel(vec![id_exp("_x")], vec![ret("_x"), instr_return], None).unwrap();
    assert_eq!(id_of_exp(return_exp(&block[1])).node.as_ptr(), ptr_body);
}
    ...
    for text_input in texts {
        for text_bound in texts {
            for text_used in texts {
                let mut exp_input = id_exp(text_input);
                let ExpKind::Id(id_input) = &mut exp_input.node else { unreachable!() };
                id_input.span = span(31);
