...
fn test_equality_preserves_order_total_and_ignores_spans_and_subproofs() {
    ...
    instr_b.span = span(2);
    let ast_ol::InstrKind::Case(instr_case) = &mut instr_b.node else { panic!("expected case") };
...
fn test_case_branch_order_matters() {
    let case_a = ast_ol::Case { guard: Guard::Bool(true), block: vec![ret("a")] };
    let case_b = ast_ol::Case { guard: Guard::Bool(false), block: vec![ret("b")] };
