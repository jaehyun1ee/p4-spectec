...
fn test_nested_bindings_avoid_upstream_guard_names_and_keep_iterator_roles() {
    ...
    let instr_case = instr(InstrKind::Case(CaseInstr {
        exp: id_exp("case"),
        cases: vec![Case { guard: Guard::Mem(id_exp("x")), block: vec![instr_let] }],
        total: false,
    }));
    let (_, block, _) =
        rename_tick::apply_rel(&mut false, (vec![id_exp("x'")], vec![instr_case], None)).unwrap();
    ...
    let id_bind = id_of_exp(&instr_let.exp_l);
    ...
    assert_eq!(id_of_exp(return_exp(&instr_let.block[0])), id_bind);
    let InstrKind::Let(instr_inner) = &instr_let.block[1].node else {
        panic!("expected nested let")
    };
    assert_eq!(id_of_exp(&instr_inner.exp_r), id_bind);
    assert_eq!(id_of_exp(&instr_inner.exp_l).node, "x'''");
    assert_eq!(id_of_exp(return_exp(&instr_inner.block[1])), id_bind);
}

#[test]
fn test_rule_output_renaming_keeps_input_and_locations() {
    let mut exp_output = id_exp("out'''");
    ...
    assert_eq!(id_of_exp(exps[0]).node, "input");
    assert_eq!(id_of_exp(exps[1]).node, "out");
    assert_eq!(id_of_exp(exps[1]).span, span(23));
    assert_eq!(id_of_exp(return_exp(&instr_rule.block[0])), id_of_exp(exps[1]));
...
fn test_unticked_binding_keeps_notation_storage_and_input_spans() {
    let exp_case = crate::note_phrase! {
        node: crate::lang::il::ast::ExpKind::Case(Box::new(Mixfix::Arg(id_exp("payload")))),
    ...
    let (_, block, _) =
        rename_tick::apply_rel(&mut false, (vec![id_exp("input")], block, None)).unwrap();
...
fn test_tick_gap_uses_whole_names_and_preserves_existing_binding() {
    ...
    assert_eq!(id_of_exp(&instr_a.exp_l).node, "é''");
    assert_eq!(id_of_exp(&instr_b.exp_l).node, "z'");
    assert_eq!(id_of_exp(return_exp(&instr_b.block[0])), id_of_exp(&instr_b.exp_l));
}
