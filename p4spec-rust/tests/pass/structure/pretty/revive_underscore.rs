...
fn test_unused_inputs_stay_underscored_and_let_scope_shadows_input() {
    ...
    assert_eq!(var_id(return_exp(&instr_let.block[0])), var_id(&instr_let.exp_l));
}
...
fn test_rule_iterator_scopes_and_used_guard_survive_nested_branches() {
    let instr_rule = instr(InstrKind::Rule(RuleInstr {
        id: id("rel"),
        not_exp: Mixfix::Seq(vec![Mixfix::Arg(variable("_input")), Mixfix::Arg(variable("_out"))]),
    ...
    let instr_case = instr(InstrKind::Case(CaseInstr {
        exp: variable("_input"),
        total: true,
        cases: vec![Case { guard: Guard::Mem(variable("_input")), block: vec![instr_rule] }],
    }));
...
fn test_let_iterator_bound_and_binding_names_follow_separate_scopes() {
    ...
    let (exps_match, block, _) =
        revive_underscore::apply_rel(&mut false, (vec![variable("_input")], vec![instr_let], None))
            .unwrap();
    let id_input = var_id(&exps_match[0]);
    let InstrKind::Let(instr_let) = &block[0].node else { panic!("expected let") };
...
fn test_progress_tracks_iterator_only_uses_but_not_unused_candidates() {
    ...
    let (exps_match, block, _) =
        revive_underscore::apply_rel(&mut changed, (vec![variable("_x")], vec![instr_if], None))
            .unwrap();
    assert!(changed);
