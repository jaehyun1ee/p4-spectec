...
fn test_nested_order_and_debug_barrier() {
    ...
    let block = apply(vec![group(vec![ret("head"), instr_if]), instr_debug.clone(), ret("tail")]);
    assert_eq!(block[0], ret("head"));
    let InstrKind::If(instr_if) = &block[1].node else { panic!("expected if") };
    assert_eq!(instr_if.block, vec![ret("a"), ret("b"), ret("c")]);
    assert_eq!(instr_if.iter_exps, vec![ExpIter { iter: Iter::List, vars: vec![] }]);
