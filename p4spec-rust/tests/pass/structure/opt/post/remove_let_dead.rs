...
fn test_liveness_uses_each_container_expression_and_branch() {
    let blocks = vec![
        vec![instr(InstrKind::If(IfInstr { exp: id_exp("x"), iter_exps: vec![], block: vec![] }))],
        vec![instr(InstrKind::Hold(HoldInstr {
            id: id("R"),
            not_exp: Mixfix::Arg(id_exp("other")),
            iter_exps: vec![],
            block_hold: vec![],
        ...
        vec![instr(InstrKind::Case(CaseInstr {
            exp: id_exp("other"),
            cases: vec![Case { guard: Guard::Mem(id_exp("x")), block: vec![] }],
            total: false,
        }))],
...
fn test_downstream_uses_are_computed_before_recursive_deletion() {
    let exp_tuple = crate::note_phrase!(node: ExpKind::Tuple(vec![id_exp("x")]), note: TypKind::Tuple(vec![]), span: span(8));
...
fn test_update_path_call_keeps_dead_binding() {
    ...
    let exp_call = crate::note_phrase!(node: ExpKind::Call(id("index"), vec![], vec![]), note: TypKind::Num(crate::lang::common::prim::num::Typ::Nat), span: span(8));
    ...
    let exp_update = crate::note_phrase!(node: ExpKind::Upd(Box::new(id_exp("base")), Box::new(path), Box::new(literal())), note: TypKind::Bool, span: span(11));
