...
fn case(arena: &mut ValueArena, shape: &str, args: Vec<Value>, typ: &str) -> Value {
    ...
    make::case_(arena, &typ, value_case, Span::default()).unwrap()
}
...
fn run_static_assert(names_param: &[&str]) -> (Value, Value) {
    ...
    let value_check = {
        let (value_arg0, value_arg1, value_arg2) = (
            "_B bool",
            vec![make::bool(runner.arena_mut(), true, Span::default()).unwrap()],
            "value",
        );
        case(runner.arena_mut(), value_arg0, value_arg1, value_arg2)
    };
    let value_message = {
        let (value_arg0, value_arg1, value_arg2) = (
            "'\"' text '\"'",
    ...
    let values_name = names_param
        ...
        .collect();
    let value_names =
        make::list(runner.arena_mut(), &typ_names, values_name, Span::default()).unwrap();
...
fn test_static_assert_returns_true() {
    for names_param in [&["check"][..], &["check", "message"][..]] {
        ...
        assert!(std::cmp::PartialEq::eq(&value_check, &value_result));
    }
