...
impl HasCall for ExpKind {
    fn has_call(&self) -> bool {
        match self {
            Self::Bool(_) | Self::Num(_) | Self::Text(_) | Self::Id(_) => false,
            Self::Un(_, _, exp)
            | Self::UpCast(_, exp)
            | Self::DownCast(_, exp)
            | Self::Sub(exp, _, _)
            ...
            Self::Str(fields) => fields.iter().any(|ExpField { exp, .. }| exp.has_call()),
