...
use super::{LexError, describe_utf8_error, make_diagnostic};
...
pub(crate) fn text_encoding_invalid(span: Span, error: &std::string::FromUtf8Error) -> LexError {
    ...
    diagnostic.notes.push(format!("Hex escapes encode bytes, not Unicode characters. If you intended U+{byte:04X}, use `\\u{{{byte:X}}}`; otherwise supply a complete UTF-8 byte sequence."));
    Box::new(diagnostic.into())
}
...
pub(crate) fn text_escape_codepoint_invalid(span: Span, digits: &str) -> LexError {
    ...
    diagnostic.notes.push("Valid Unicode scalar values are U+0000–U+D7FF and U+E000–U+10FFFF; U+D800–U+DFFF are reserved for UTF-16 surrogates.".to_owned());
    Box::new(diagnostic.into())
}
