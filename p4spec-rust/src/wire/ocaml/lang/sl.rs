...
use super::{
    super::{
        ...
        encode_option, field, integer, object, on_codec_stack, variant,
    },
    el, il,
};
...
fn encode_instr_kind(instr: &InstrKind) -> Value {
    match instr {
        ...
        InstrKind::Group(GroupInstr {
            ...
            encode_list(exps, il::encode_exp),
            encode_block(block)
        ]),
...
fn decode_block(value: &Value) -> Result<ast::Block, DecodeError> {
    decode_list(value, decode_instr)
}

fn encode_block(block: &ast::Block) -> Value {
    encode_list(block, encode_instr)
}
...
fn encode_extern_rel(relation: &ast::ExternRel) -> Value {
    json!([
        ...
        encode_list(&relation.exps_input, il::encode_exp),
        encode_list(&relation.hints, el::encode_hint)
    ])
}
...
fn encode_extern_func(function: &ast::ExternFunc) -> Value {
    json!([
        ...
        encode_list(&function.hints, el::encode_hint)
    ])
}
...
fn encode_table_row(row: &ast::TableRow) -> Value {
    json!([
        encode_list(&row.exps_input, il::encode_exp),
...
fn encode_table_func(function: &ast::TableFunc) -> Value {
    json!([
        ...
        encode_list(&function.params, encode_param),
        il::encode_typ(&function.typ),
        encode_list(&function.table_rows, encode_table_row),
        encode_list(&function.hints, el::encode_hint)
    ])
}
