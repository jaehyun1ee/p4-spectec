...
fn decode_param(json: &json) -> Result<ast::Param, DecodeError> {
    source::decode_phrase(json, |json| {
        ...
        match (tag, fields) {
            ("ExpP", [typ, exp]) => {
                Ok(ParamKind::Exp(il::decode_typ(typ)?, Box::new(il::decode_exp(exp)?)))
            }
...
fn decode_hold_case(json: &json) -> Result<HoldCase, DecodeError> {
    ...
    match (tag, fields) {
        ("BothH", [block_hold, block_not_hold]) => {
            Ok(HoldCase::Both(decode_block(block_hold)?, decode_block(block_not_hold)?))
        }
...
fn encode_hold_case(case: &HoldCase) -> json {
    match case {
        HoldCase::Both(block_hold, block_not_hold) => {
            json!(["BothH", encode_block(block_hold), encode_block(block_not_hold)])
        }
...
fn decode_case(json: &json) -> Result<ast::Case, DecodeError> {
    match array(json)? {
        [guard, block] => {
            Ok(ast::Case { guard: decode_guard(guard)?, block: decode_block(block)? })
        }
...
fn decode_guard(json: &json) -> Result<Guard, DecodeError> {
    ...
    match (tag, fields) {
        ...
        ("CmpG", [op, typ, exp]) => {
            Ok(Guard::Cmp(il::decode_cmp_op(op)?, il::decode_op_typ(typ)?, il::decode_exp(exp)?))
        }
        ("SubG", [typ, subcheck]) => {
...
fn encode_guard(guard: &Guard) -> json {
    match guard {
        ...
        Guard::Cmp(op, typ, exp) => {
            json!(["CmpG", il::encode_cmp_op(*op), il::encode_op_typ(*typ), il::encode_exp(exp)])
        }
...
fn decode_instr_kind(json: &json) -> Result<InstrKind, DecodeError> {
    ...
    match (tag, fields) {
        ...
        ("ReturnI", [exp]) => Ok(InstrKind::Return(ReturnInstr { exp: il::decode_exp(exp)? })),
...
fn encode_instr_kind(instr: &InstrKind) -> json {
    match instr {
        ...
        InstrKind::Case(CaseInstr { exp, cases, dangle }) => {
            json!(["CaseI", il::encode_exp(exp), il::encode_list(cases, encode_case), dangle])
        }
        InstrKind::Group(GroupInstr { id, rel_signature, exps, block }) => json!([
            "GroupI",
            il::encode_id(id),
        ...
        InstrKind::Let(LetInstr { exp_l, exp_r, iter_instrs: iters, block }) => json!([
            "LetI",
            il::encode_exp(exp_l),
            il::encode_exp(exp_r),
        ...
        InstrKind::Result(ResultInstr { rel_signature, exps }) => json!([
            "ResultI",
            encode_rel_signature(rel_signature),
            il::encode_list(exps, il::encode_exp)
        ]),
