...
pub(crate) fn eval_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    ...
    ctx.trace_exp(exp, result)
}

pub(crate) fn eval_exps<
    T: Borrow<ast::Exp>,
    Ctx: EvalContext<Iface, Exn>,
    Iface: Interface,
    Exn: Extern,
>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ...
    for exp in exps {
        values.push(backtrack!(eval_exp(runner, ctx, exp.borrow())));
    }
    Backtrack::Ok(values)
}
...
fn eval_un_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    op: &ast::UnOp,
    ...
    let value = match op {
        ast::UnOp::Bool(boolean::UnOp::Not) => {
            let bool = !backtrack_from_result!(get::bool(runner.arena(), &value), span);
            backtrack_from_result!(make::bool(runner.arena_mut(), bool, Span::default()), span)
        }
        ast::UnOp::Num(op) => {
            let num = backtrack_from_result!(get::num(runner.arena(), &value), span);
            let num = num::un(*op, num);
            backtrack_from_result!(make::num(runner.arena_mut(), num, Span::default()), span)
        }
    };
    Backtrack::Ok(value)
}
...
fn eval_bin_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    op: &ast::BinOp,
    ...
    let value = match op {
        ast::BinOp::Bool(op) => {
            let bool_l = backtrack_from_result!(get::bool(runner.arena(), &value_l), span);
            let bool_r = backtrack_from_result!(get::bool(runner.arena(), &value_r), span);
            let result = match op {
            ...
            backtrack_from_result!(make::bool(runner.arena_mut(), result, Span::default()), span)
        }
        ast::BinOp::Num(op) => {
...
fn eval_cmp_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    op: &ast::CmpOp,
...
fn eval_upcast_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    typ: &ast::Typ,
    exp_inner: &ast::Exp,
...
fn eval_downcast_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    typ: &ast::Typ,
    exp_inner: &ast::Exp,
...
fn eval_sub_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    exp_inner: &ast::Exp,
    ...
    let value =
        backtrack_from_result!(make::bool(runner.arena_mut(), matches, Span::default()), span);
    Backtrack::Ok(value)
}

// - Match expression

fn eval_match_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    exp_inner: &ast::Exp,
    pattern: &ast::Pattern,
...
fn eval_tuple_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    typ: &Rc<ast::TypKind>,
    ...
    let values = backtrack!(eval_exps(runner, ctx, exps));
    let value = backtrack_from_result!(
...
fn eval_case_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    typ: &Rc<ast::TypKind>,
    ...
    for exp in not_exp.args() {
        values.push(backtrack!(eval_exp(runner, ctx, exp)));
    }
...
fn eval_str_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    typ: &Rc<ast::TypKind>,
    ...
    for (atom, exp) in exp_fields {
        value_fields.push((atom.clone(), backtrack!(eval_exp(runner, ctx, exp))));
    }
    let value = backtrack_from_result!(
        make::structure(runner.arena_mut(), typ.clone(), value_fields, Span::default()),
        span
    );
    Backtrack::Ok(value)
}
...
fn eval_opt_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    typ: &Rc<ast::TypKind>,
    ...
    let value = match exp {
        Some(exp) => Some(backtrack!(eval_exp(runner, ctx, exp))),
        None => None,
    };
    let value = backtrack_from_result!(
...
fn eval_list_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    typ: &Rc<ast::TypKind>,
...
fn eval_cons_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    typ: &Rc<ast::TypKind>,
    ...
    let value_head = backtrack!(eval_exp(runner, ctx, exp_head));
    let value_tail = backtrack!(eval_exp(runner, ctx, exp_tail));
    let values_tail = backtrack_from_result!(get::list(runner.arena(), &value_tail), span);
...
fn eval_cat_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    typ: &Rc<ast::TypKind>,
    ...
    let value = match (runner.arena().kind(&value_l), runner.arena().kind(&value_r)) {
        (ValueKind::Text(text_l), ValueKind::Text(text_r)) => {
            ...
            backtrack_from_result!(make::text(runner.arena_mut(), text, Span::default()), span)
        }
...
fn eval_mem_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    exp_elem: &ast::Exp,
    ...
    let value =
        backtrack_from_result!(make::bool(runner.arena_mut(), contains, Span::default()), span);
    Backtrack::Ok(value)
}

// - Length expression

fn eval_len_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    exp_inner: &ast::Exp,
) -> Backtrack<Value> {
...
fn eval_dot_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    exp_base: &ast::Exp,
    ...
    let value = backtrack!(eval_exp(runner, ctx, exp_base));
    ops::access_dot(runner.arena(), &value, atom, span)
}

// - Index expression

fn eval_idx_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    exp_base: &ast::Exp,
    exp_idx: &ast::Exp,
    ...
    ops::access_index(runner.arena_mut(), &value, &value_idx, &exp_base.span, &exp_idx.span)
}

// - Slice expression

fn eval_slice_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    span: &Span,
    typ: &Rc<ast::TypKind>,
...
fn eval_upd_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    exp_base: &ast::Exp,
    path: &ast::Path,
    ...
    let value_base = backtrack!(eval_exp(runner, ctx, exp_base));
    let value_upd = backtrack!(eval_exp(runner, ctx, exp_upd));
    eval_update_path(runner, ctx, &value_base, path, value_upd)
}

// - Call expression

fn eval_call_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    id: &ast::Id,
    targs: &[ast::Typ],
    ...
    for targ in targs {
        targs_subst.push(backtrack_from_result!(subst_typ(&theta, targ), &targ.span));
    }
    let values = backtrack!(eval_args(runner, ctx, args));
    ctx.invoke_func(runner, id, &targs_subst, &values)
}

// - Iteration expression

fn eval_iter_exp<Ctx: EvalContext<Iface, Exn>, Iface: Interface, Exn: Extern>(
    runner: &mut RunnerContext<'_, Ctx::Interp, Iface, Exn>,
    ctx: &Ctx,
    exp: &ast::Exp,
    exp_inner: &ast::Exp,
    ...
    let value = match iter {
        ast::Iter::Opt => {
            let value = backtrack!(ctx.map_opt(runner, span, vars, |runner, ctx_sub| {
        ...
        ast::Iter::List => {
            let values = backtrack!(ctx.map_list(runner, span, vars, |runner, ctx_sub| {
                eval_exp(runner, ctx_sub, exp_inner)
            }));
            backtrack_from_result!(
                make::list(runner.arena_mut(), typ.clone(), values, Span::default()),
