...
pub(super) fn cast_up(
    ...
    let result = match &typ.node {
        ast::TypKind::Num(num::Typ::Int) => {
            ...
            match num {
                num::Number::Nat(num) => back!(Backtrack::from_result(
                    make::int(arena, num.as_bigint().clone(), Span::default()),
                    &Span::default()
                )),
        ...
        ast::TypKind::Tuple(typs) => {
            ...
            back!(Backtrack::from_result(
                make::tuple(arena, typ, values_cast, Span::default()),
                &Span::default()
            ))
        }
        ast::TypKind::Iter(typ_inner, ast::Iter::Opt) => {
            ...
            back!(Backtrack::from_result(
                make::opt(arena, typ_inner, value, Span::default()),
                &Span::default()
            ))
        }
        ast::TypKind::Iter(typ_inner, ast::Iter::List) => {
            ...
            back!(Backtrack::from_result(
                make::list(arena, typ_inner, values_cast, Span::default()),
                &Span::default()
            ))
        }
        _ => value,
    ...
    match arena.kind(value_base) {
        ...
        ValueKind::List(values) => {
            ...
            Backtrack::Ok(back!(Backtrack::from_result(
                make::list(arena, typ, values, Span::default()),
                &Span::default()
            )))
        }
        _ => unreachable!(),
...
pub(super) fn update_index(
    ...
    let value = match arena.kind(value_base) {
        ValueKind::Text(text) => {
            ...
            let value_idx_slice = back!(Backtrack::from_result(
                make::int(arena, (0).into(), Span::default()),
                &Span::default()
            ));
            let value_len_slice = back!(Backtrack::from_result(
            ...
            let value_len_slice = back!(Backtrack::from_result(
                make::int(arena, (text_len as i64 - (idx + 1)).into(), Span::default()),
                &Span::default()
            ));
...
pub(super) fn update_slice(
    ...
    let value = match arena.kind(value_base) {
        ValueKind::Text(text) => {
            ...
            let value_len_slice = back!(Backtrack::from_result(
                make::int(arena, (text_len as i64 - (end)).into(), Span::default()),
                &Span::default()
            ));
            let value_r = back!(access_slice(
                arena,
