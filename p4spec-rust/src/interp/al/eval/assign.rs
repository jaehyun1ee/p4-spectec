    ...
    match (&exp.node, arena.kind(&value)) {
        ...
        _ => Backtrack::err(
            ...
            ErrorKind::Assign(AssignErrorKind::Mismatch {
                ...
                value: crate::lang::il::print::string_of_value(arena, &value),
            }),
        ),
    }
}
...
fn assign_cons_exp<'global>(
    ...
    let value_tail = back!(Backtrack::from_result(
        make::list(arena, &typ, values_tail.to_vec(), Span::default()),
        &Span::default()
    ));
...
fn assign_iter_exp<'global>(
    ...
    match iter {
        ast::Iter::Opt => {
            let value_inner = back!(Backtrack::from_result(get::opt(arena, &value), span)).copied();
            ...
            for var in vars {
                ...
                let value_sub = back!(Backtrack::from_result(
                    make::opt(arena, &typ, value_sub, Span::default()),
                    &Span::default()
                ));
...
fn assign_def_arg<'global>(
    ...
    let (_, func) = back!(Backtrack::from_result(
        ctx_caller.find_func(&arena.resolve_phrase(id_func)),
        arena.location(id_func.span)
    ));
