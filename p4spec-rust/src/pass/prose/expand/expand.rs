...
/// Expands every executable body owned by a definition.
...
fn expand_instr(ids_used: &mut IdSet, instr_sl: sl::Instr) -> Result<sl::Instr, ProseError> {
    // Expand nested blocks before lifting calls from this instruction
    let mut instr_sl = expand_instr_subblocks(ids_used, instr_sl)?;
    let mut calls_lifted = Vec::new();
    // Collect calls from left to right until the instruction is stable
    ...
    // Rebuild nested lets from the last lifted call to the first
    for call_lifted in calls_lifted.into_iter().rev() {
        instr_sl = call_lifted.wrap_instr(instr_sl);
    }
    // Expand the new let bodies introduced by wrapping
    expand_instr(ids_used, instr_sl)
}

fn expand_instr_subblocks(
    ids_used: &mut IdSet,
    instr_sl: sl::Instr,
) -> Result<sl::Instr, ProseError> {
...
/// Lifts the first eligible call directly owned by an instruction.
fn lift_instr_call(
