...
pub fn rev_(
    ...
    values.reverse();
    let value = make::list(arena, &typ_list, values, Span::default())?;
    Ok(value)
}
...
pub fn concat_(
    ...
    let value = make::list(arena, &typ_list, concatenated, Span::default())?;
    Ok(value)
}
...
pub fn distinct_(
    ...
    sorted.sort_by(|left, right| arena.compare(left, right));
    let all_distinct = sorted
        .windows(2)
        .all(|pair| !arena.equal(&pair[0], &pair[1]));
...
pub fn partition_(
    ...
    let values = list_of_value(arena, value_list)?.to_vec();
...
pub fn assoc_(
    ...
    for pair in list_of_value(arena, value_list)? {
        ...
        if found.is_none() && arena.equal(value, &pair[0]) {
            found = Some(pair[1]);
        }
    }
    let typ_opt = typ::make::opt(typ_value.clone());
    let value = make::opt(arena, &typ_opt, found, Span::default())?;
    Ok(value)
}
...
pub fn transpose_(
    ...
    for column in columns {
        let value_row = make::list(arena, &typ_list, column, Span::default())?;
        value_rows.push(value_row);
    }
    let value = make::list(arena, &typ_matrix, value_rows, Span::default())?;
    Ok(value)
}
