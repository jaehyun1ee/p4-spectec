...
fn text_of_value<'a>(span: &Region, value: &'a ValueRef) -> Result<&'a str, BuiltinError> {
    get::text(value).map_err(|error| BuiltinError::new(span.clone(), error.to_string()))
}

fn numeric_text(span: &Region, value: &ValueRef) -> Result<String, BuiltinError> {
    match get::num(value).map_err(|error| BuiltinError::new(span.clone(), error.to_string()))? {
        num::T::Nat(value) | num::T::Int(value) => Ok(value.to_string()),
    }
}
...
pub fn int_to_text(
    ...
    return_value(add, make::text(text, Region::none()))
}
...
pub fn split_text(
    ...
    let parts = text
        .split(separator)
        .map(|part| make::text(part.to_owned(), Region::none()))
        .collect();
    let list_type = make_type::list_type(make_type::bool_type());
    return_value(add, make::list(&list_type, parts, Region::none()))
}
...
pub fn strip_prefix(
    ...
    return_value(add, make::text(text.to_owned(), Region::none()))
}
