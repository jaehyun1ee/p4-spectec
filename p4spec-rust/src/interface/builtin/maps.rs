...
fn make_pair(
    ...
    Ok(make::case_(arena, &typ, value_case, Span::default())?)
}
...
fn map_update(
    ...
    for pair in map {
        let matching = get::case(arena, pair).ok().is_some_and(|value_case| {
            ...
            let args = value_case.args();
            matches!(args.as_slice(), [value_key, _] if arena.equal(value_key, key))
        });
        if !found && matching {
...
fn map_of_value(arena: &ValueArena, value: &Value) -> Result<ValueMap, BuiltinError> {
    ...
    if !value_case.eq_shape(map_mixop.as_ref()) {
...
fn value_of_map(
    ...
    let value_pairs = make::list(arena, &typ_pairs, map, Span::default())?;
    ...
    let value = make::case_(arena, &typ, value_case, Span::default())?;
    Ok(value)
}
...
pub fn find_map(
    ...
    let value = make::opt(arena, &typ_opt, value_opt, Span::default())?;
    Ok(value)
}
