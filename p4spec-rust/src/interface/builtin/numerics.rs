...
fn bits_of_value(span: &Region, value: &Value) -> Result<Vec<bool>, BuiltinError> {
    get::list(value)
...
fn value_of_bits(add: &mut dyn FnMut(ValueRef), bits: Vec<bool>) -> BuiltinResult {
    let typ = make_type::var_type(Spanned::new("bit".to_owned(), Region::none()), Vec::new());
    let values_bit = bits
        .into_iter()
        .map(|bit| make::bool(bit, Region::none()))
        .collect();
    return_value(add, make::list(&typ, values_bit, Region::none()))
}
...
fn pow2_value(span: &Region, width: &BigInt) -> Result<BigInt, BuiltinError> {
    if width <= &BigInt::zero() {
...
pub fn bits_to_int_signed(
    ...
    let Some(sign) = bits.first() else {
        return Err(BuiltinError::new(Region::none(), "empty bit array"));
    };
